...
class UniSpeechFlashAttention2(UniSpeechAttention):
    ...
    def forward(
        ...
        return  attn_weights, past_key_value
...
class UniSpeechEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if not skip_the_layer or False:
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if self._use_flash_attention_2:
                ...
                # extend attention_mask
                attention_mask = 1.0 - attention_mask[:, None, None, :].to(dtype=hidden_states.dtype)
                attention_mask = attention_mask * ops.finfo(hidden_states.dtype).min
                attention_mask = attention_mask.broadcast_to(
                    (attention_mask.shape[0], 1, attention_mask.shape[-1], attention_mask.shape[-1])
                )
...
UNISPEECH_START_DOCSTRING = r"""
    ...
    This model is a PyTorch [ops.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) sub-class. Use
...
class UniSpeechModel(UniSpeechPreTrainedModel):
    def __init__(self, config: UniSpeechConfig):
        ...
        if config.mask_time_prob > 0.0 or config.mask_feature_prob > 0.0:
            self.masked_spec_embed = nn.Parameter(mindspore.Tensor(config.hidden_size))
    ...
    def _mask_hidden_states(
        ...
        if self.config.mask_feature_prob > 0 and self.training:
            ...
            mask_feature_indices = mindspore.tensor(mask_feature_indices,dtype=ops.bool)
...
class UniSpeechForPreTraining(UniSpeechPreTrainedModel):
    ...
    def compute_contrastive_logits(
        ...
        logits = ops.cosine_similarity(predicted_features.float(), target_features.float(), axis=-1)
    ...
    def forward(
        ...
        ```python
        >>> import mindspore
        >>> from mindnlp.transformers import AutoFeatureExtractor, UniSpeechForPreTraining
        ...
        prob_replace_matrix = ops.empty(transformer_features.shape[0], transformer_features.shape[1]).fill_(
            self.config.replace_prob
        )
        prob_replace_matrix = prob_replace_matrix.swapaxes(0, 1)
        sampled_replace_matrix = ops.bernoulli(prob_replace_matrix).bool()
